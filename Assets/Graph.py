...
class Graph:
    ...
    @staticmethod
    def render_2_actions_graph(game: Game, Agents_list: list[Agent], normalise:bool = True):
        A, B = game.get_bi_matrix_seperated()
        ...
        for i in range(x.shape[0]):
            for j in range(x.shape[1]):
                tmp_x = x[j,i]*((A@vector_agent_2[j,i])[0] - vector_agent_1[j,i].T@A@vector_agent_2[j,i])
                tmp_y = y[j,i]*((B@vector_agent_1[j,i])[0] - vector_agent_2[j,i].T@B@vector_agent_1[j,i])
                if normalise:
                    norm = np.linalg.norm([tmp_x,tmp_y])
                    tmp_x = tmp_x/norm if not np.isclose(norm, 0) else tmp_x
                    tmp_y = tmp_y / norm if not np.isclose(norm, 0) else tmp_y
    ...
    @staticmethod
    def render_combined_2_actions_graph(game: Game, all_traces: list[list[list[list]]], initial_Q_values_list, alpha, gamma, tau, learning_name, normalise: bool = True):
        # kappa = 1 -> Q learning
